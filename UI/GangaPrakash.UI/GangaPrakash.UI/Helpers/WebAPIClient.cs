...
namespace GangaPrakash.UI
{
    public class WebAPIClient
    {
        ...
        public static async Task<T> PutAsync<T>(String Uri, String Path, T result) where T : new()
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(Uri);
                ...
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage response;
                response = await client.PutAsJsonAsync(Path, result);
                if (response.IsSuccessStatusCode)
                {
                    result = await response.Content.ReadAsAsync<T>();
                }
        ...
        public static async Task<T> PutAsync<T>(String Uri, String Path, T result, String AccessToken = null) where T : new()
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(Uri);
